...
class GFJeebAdmin
{
    ...
    public function __construct($plugin)
    {
        ...
        if (true === class_exists('GFCommon')) {
            if (version_compare(GFCommon::$version, '1.6.99999', '<')) {
                // pre-v1.7 settings
                $this->settingsURL = admin_url('admin.php?page=gf_settings&addon=Jeeb+Payments');
            } else {
                // post-v1.7 settings
                $this->settingsURL = admin_url('admin.php?page=gf_settings&subview=Jeeb+Payments');
            }
        }
        ...
        add_action('plugin_action_links_' . GFJEEB_PLUGIN_NAME, array($this, 'addPluginActionLinks'));
        ...
        if (true === isset($_GET['page'])) {
            switch ($_GET['page']) {
                case 'gf_settings':
                    ...
                    RGForms::add_settings_page('Jeeb Payments', array($this, 'optionsAdmin'));
                    break;
                default:
                    // not used
            }
    ...
    {
        wp_enqueue_style('gfjeeb-admin', $this->plugin->urlBase . 'style-admin.css', false, GFJEEB_PLUGIN_VERSION);
    }
    ...
    public function actionAdminNotices()
    {
        if (self::isGfActive() == false) {
            $this->plugin->showError('Gravity Forms Jeeb Payments requires <a href="http://www.gravityforms.com/">Gravity Forms</a> to be installed and activated.');
        }
    }
    ...
    */
    ...
    {
        if (true === isset($file) && $file == GFJEEB_PLUGIN_NAME) {
            $links[] = '<a href="https://support.jeeb.com">' . __('Get help') . '</a>';
            $links[] = '<a href="https://www.jeeb.io">' . __('Jeeb.com') . '</a>';
        }
    ...
    public function gformEntryInfo($form_id, $lead)
    {
        if (true === isset($lead) && false === empty($lead)) {
            ...
            error_log('[ERROR] In GFJeebAdmin::gformEntryInfo(): Missing or invalid $lead parameter.');
            throw new \Exception('An error occurred in the Jeeb Payment plugin: Missing or invalid $lead parameter in the gformEntryInfo() function.');
        }
    }
    ...
    {
        $admin = new GFJeebOptionsAdmin($this->plugin, 'gfjeeb-options', $this->settingsURL);

        if (true === isset($admin) && false === empty($admin)) {
            $admin->process();
        } else {
            error_log('[ERROR] In GFJeebAdmin::optionsAdmin(): Could not create a new GFJeebOptionsAdmin object.');
            throw new \Exception('An error occurred in the Jeeb Payment plugin: Could not create a new GFJeebOptionsAdmin object.');
        }
    }
